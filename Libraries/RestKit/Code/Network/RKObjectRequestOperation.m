    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(HTTPOperationDidStart:)
                                                     name:CAFNetworkingOperationDidStartNotification
                                                   object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(HTTPOperationDidFinish:)
                                                     name:CAFNetworkingOperationDidFinishNotification
                                                   object:nil];
    }
...
- (void)HTTPOperationDidStart:(NSNotification *)notification
{
    ...
    if (![operation isKindOfClass:[CAFHTTPRequestOperation class]]) return;
...
{
    #if __IPHONE_OS_VERSION_MIN_REQUIRED
        [[CAFNetworkActivityIndicatorManager sharedManager] incrementActivityCount];
    #endif
}
...
{
    #if __IPHONE_OS_VERSION_MIN_REQUIRED
        [[CAFNetworkActivityIndicatorManager sharedManager] decrementActivityCount];
    #endif
}
...
- (void)execute
{
    ...
    [self.HTTPRequestOperation setCompletionBlockWithSuccess:^(CAFHTTPRequestOperation *operation, id responseObject) {
        if (weakSelf.isCancelled) {
            [weakSelf.stateMachine finish];
            return;
        }
